...
use crate::{
    crate_prelude::*,
    hir::HirNode,
    ty::{bit_size_of_type, Domain, PackedType, SbvType, Sign, Type, TypeKind, UnpackedType},
    value::ValueKind,
...
fn self_determined_expr_type<'gcx>(
    ...
    match expr.kind {
        ...
        hir::ExprKind::Index(target, mode) => {
            ...
            Some(if target_ty.dims().next().is_some() {
                target_ty.pop_dim(cx).unwrap()
            } else {
                match target_ty.get_simple_bit_vector() {
                    Some(ty) => ty.forget().change_size(width.unwrap()).to_unpacked(cx),
                    None => {
                        cx.emit(
                            DiagBuilder2::error(
...
fn unify_operator_types<'gcx>(
    ...
    // Construct the type.
    width.map(|w| SbvType::new(domain, sign, w).to_unpacked(cx))
}
...
fn type_context_imposed_by_positional_pattern<'gcx>(
    ...
    match *ty.to_legacy(cx).resolve_name() {
        ...
        TypeKind::BitVector { sign, domain, .. } => Some(ty.into()),
